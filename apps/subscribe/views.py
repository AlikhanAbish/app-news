...
class PinnedPostView(generics.RetrieveUpdateDestroyAPIView):
    """Детали закрепленного поста пользователя"""

    serializer_class = PinnedPostSerializer
    permission_classes = [permissions.IsAuthenticated]

    def get_object(self):
        """Возвращает закрепленный пост пользователя или None"""
    ...
    def update(self, request, *args, **kwargs):
        """Обновляет закрепленный пост пользователя"""
    ...
    if serializer.is_valid():
        ...
        try:
            with transaction.atomic():
                ...
                # Удаляем существующий закрепленный пост, если он есть
...
def pinned_posts_list(request):
    ...
    pinned_posts = PinnedPost.objects.select_related(
        ...
        user__subscription__end_date__gte=timezone.now(),
    ...
    for pinned_post in pinned_posts:
        ...
        posts_data.append({
            ...
            'author': {
                ...
                'full-name': post.author.full-name,
            },
